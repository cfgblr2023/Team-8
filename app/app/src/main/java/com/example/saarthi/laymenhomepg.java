    ...
    private void fetchHearingDateTime() {
        databaseRef.child("hearings").child("date").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if (hearingDate != null) {
                    showHearingDialog("Class Date and Time", "Date and time: " + hearingDate);
                } else {
                    showHearingDialog("Class Date and Time", "No class date available");
                }
            }
        ...
        ref2.child("court_orders").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                for (DataSnapshot courtOrderSnapshot : dataSnapshot.getChildren()) {
                    ...
                    String message ="Roll no: " + cnr + "\n"
                            + "Name: " + convictName + "\n"
                            + "Rating: " + judgement + "\n"
                            + "Subject: " + courtAddress + "\n"
                            + "Level: " + courtLevel;

                    showCourtOrderDialog("Feedback message", message);
                }
            }

            @Override
            public void onCancelled(DatabaseError databaseError) {
                showCourtOrderDialog("Feedback details", "Could not fetch the details");
            }
        });
    }
